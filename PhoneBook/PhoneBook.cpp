...
bool PhoneBook:: isCheckedPhone(char* checkPhone) {
    int size = 0;
...
void PhoneBook:: AskInformation() {
    if (global::index < MAX) {
            ...
            while (!isCheckedPhone(buffWorkPhone)) {
                cout << "Work phone number (for exp. 012 345 67 89): ";
                cin >> buffWorkPhone;
            }
            ...
            while (!isCheckedPhone(buffMobPhone)) {
                cout << "Mobile phone number (for exp. 012 345 67 89): ";
                cin >> buffMobPhone;
            }
...
void PhoneBook::AddUser(PhoneBook* user, int amount) {
    for (int i = 0; i < amount; i++) {
        user[i].AskInformation();
    }
}

void PhoneBook::AddPersonToFile(const char* filename, PhoneBook* user) {
    FILE* users;
...
void PhoneBook:: ShowUserData(const char* filename, PhoneBook* user) {
    FILE* users;
...
void PhoneBook::FindUserByFullName(PhoneBook* user) {
    if (global::index == 0)
    {
